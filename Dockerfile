...
RUN apk add --no-cache curl tar gcc g++ make autoconf libc-dev libevent-dev linux-headers perl \
    && mkdir -p /ssdb/tmp \
    && curl -Lk "https://github.com/ideawu/ssdb/archive/${VERSION}.tar.gz" | tar -xz -C /ssdb/tmp --strip-components=1 \
    && cd /ssdb/tmp \
